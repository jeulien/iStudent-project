...
Public Class TableFormSubjects
   
    Private Sub TableFormSubjects_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        FilterData("")
    ...
    Private Sub expBtn_Click(sender As Object, e As EventArgs) Handles expBtn.Click
        ...
        Else
            ...
            Dim PdfTable As New PdfPTable(subjectListView.Columns.Count)
            PdfTable.TotalWidth = 500.0F
            PdfTable.LockedWidth = True

            Dim widths(0 To subjectListView.Columns.Count - 1) As Single
            For i As Integer = 0 To subjectListView.Columns.Count - 1
                widths(i) = 1.0F
            Next
            ...
            For i As Integer = 0 To subjectListView.Columns.Count - 1
                pdfcell = New PdfPCell(New Phrase(New Chunk(subjectListView.Columns(i).HeaderText, pTable)))
                pdfcell.HorizontalAlignment = PdfPCell.ALIGN_LEFT
                PdfTable.AddCell(pdfcell)
            Next
            For i As Integer = 0 To subjectListView.Rows.Count - 2
                For j As Integer = 0 To subjectListView.Columns.Count - 1
                    pdfcell = New PdfPCell(New Phrase(subjectListView(j, i).Value.ToString(), pTable))
