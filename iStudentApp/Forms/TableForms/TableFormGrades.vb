...
Public Class TableFormGrades
   
    Private Sub TableFormGrades_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        FilterData("")
    ...
    Private Sub bind1()
        ...
        Dim command As New SqlCommand("Select * from gradeRecords order by studentNumber Asc", con)
    ...
    Private Sub FilterData(valueToSearch As String)
        ...
        Dim searchQuery As String = "SELECT * From gradeRecord Where Concat(studentNumber,subjectCode,schoolYear,semester,subjectTitle) like '%" & valueToSearch & "%'"
    ...
    Private Sub expBtn_Click(sender As Object, e As EventArgs) Handles expBtn.Click
        ...
        Else
            ...
            Dim PdfTable As New PdfPTable(studentGradeView.Columns.Count)
            PdfTable.TotalWidth = 500.0F
            PdfTable.LockedWidth = True

            Dim widths(0 To studentGradeView.Columns.Count - 1) As Single
            For i As Integer = 0 To studentGradeView.Columns.Count - 1
                widths(i) = 1.0F
            Next
            ...
            For i As Integer = 0 To studentGradeView.Columns.Count - 1
                pdfcell = New PdfPCell(New Phrase(New Chunk(studentGradeView.Columns(i).HeaderText, pTable)))
                pdfcell.HorizontalAlignment = PdfPCell.ALIGN_LEFT
                PdfTable.AddCell(pdfcell)
            Next
            For i As Integer = 0 To studentGradeView.Rows.Count - 2
                For j As Integer = 0 To studentGradeView.Columns.Count - 1
                    pdfcell = New PdfPCell(New Phrase(studentGradeView(j, i).Value.ToString(), pTable))
