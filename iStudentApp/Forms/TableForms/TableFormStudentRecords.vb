...
Public Class TableFormStudentRecords
   
    Private Sub TableFormStudentRecords_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        FilterData("")
    ...
    Public Sub FilterData(valueToSearch As String)
        ...
        Dim searchQuery As String = "SELECT * From studentRecord Where Concat(studentNumber,studentName,subjectCode,schoolYear,semester) like '%" & valueToSearch & "%'"
    ...
    Private Sub bind1()
        ...
        Dim command As New SqlCommand("Select * from studentRecord order by studentNumber Asc", con)
        Dim adapter As New SqlDataAdapter(command)
    ...
    Private Sub expBtn_Click(sender As Object, e As EventArgs) Handles expBtn.Click
        ...
        Else
            ...
            Dim PdfTable As New PdfPTable(studentRecordView.Columns.Count)
            PdfTable.TotalWidth = 500.0F
            PdfTable.LockedWidth = True

            Dim widths(0 To studentRecordView.Columns.Count - 1) As Single
            For i As Integer = 0 To studentRecordView.Columns.Count - 1
                widths(i) = 1.0F
            Next
            ...
            For i As Integer = 0 To studentRecordView.Columns.Count - 1
                pdfcell = New PdfPCell(New Phrase(New Chunk(studentRecordView.Columns(i).HeaderText, pTable)))
                pdfcell.HorizontalAlignment = PdfPCell.ALIGN_LEFT
                PdfTable.AddCell(pdfcell)
            Next
            For i As Integer = 0 To studentRecordView.Rows.Count - 2
                For j As Integer = 0 To studentRecordView.Columns.Count - 1
                    pdfcell = New PdfPCell(New Phrase(studentRecordView(j, i).Value.ToString(), pTable))
